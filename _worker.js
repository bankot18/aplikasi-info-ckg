...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/extension-logs' || url.pathname.startsWith('/api/extension-logs/')) {
      ...
      // Auto-create extension_logs table
      try {
        await env.DB.prepare(`CREATE TABLE IF NOT EXISTS extension_logs (
          id TEXT PRIMARY KEY,
          ...
          gula TEXT DEFAULT '-',
          kolesterol TEXT DEFAULT '-',
          hb TEXT DEFAULT '-',
          status_match TEXT DEFAULT 'PENDING',
          raw_json TEXT,
          created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )`).run();
        await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_ext_logs_nik ON extension_logs(nik)').run();
        await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_ext_logs_petugas ON extension_logs(petugas)').run();
      } catch (_) {}

      if (request.method === 'GET') {
        try {
          const { results } = await env.DB.prepare('SELECT * FROM extension_logs ORDER BY created_at DESC LIMIT 500').all();
          return new Response(JSON.stringify({ success: true, data: results || [] }), { headers: corsHeaders });
        } catch (err) {
          return new Response(JSON.stringify({ success: false, error: err.message }), { status: 500, headers: corsHeaders });
        }
        ...
        try {
          ...
          const stmt = env.DB.prepare(`
            INSERT INTO extension_logs (
              id, timestamp, petugas, activity_type, nik, nama, dob, usia, alamat,
              bb, tb, imt, sistol, diastol, gula, kolesterol, hb, status_match, raw_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ...
          const statements = items.map(item => {
            const id = item.id || `ext_${Date.now()}_${Math.random().toString(36).substring(2,7)}`;
            const ts = item.timestamp || new Date().toLocaleString('id-ID');
            return stmt.bind(
              id, ts, item.petugas || 'Extension User', item.activity_type || 'Pendaftaran CKG',
              item.nik || '', item.nama || item.nama_pasien || '', item.dob || '', item.usia || 0,
              item.alamat || '', item.bb || 0, item.tb || 0, item.imt || 0, item.sistol || 0,
              item.diastol || 0, item.gula || '-', item.kolesterol || '-', item.hb || '-',
              item.status_match || 'PENDING', JSON.stringify(item)
            );
          });
      ...
      if (request.method === 'DELETE') {
        try {
          await env.DB.prepare('DELETE FROM extension_logs').run();
          return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
        } catch (err) {
          return new Response(JSON.stringify({ success: false, error: err.message }), { status: 500, headers: corsHeaders });
        }
